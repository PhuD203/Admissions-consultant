...
export const authenticateToken = (options?: {
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      console.log('AUTH_MIDDLEWARE: Extracted Token:', token ? token.substring(0, 10) + '...' : 'Missing'); // LOG 3

      // Xử lý khi yêu cầu xác thực nhưng không có token
      if (options?.requireAuth !== false && !token) {
        console.log('AUTH_MIDDLEWARE: Access token required but missing. Returning 401.'); // LOG 4
      ...
      if (options?.requireAuth === false && !token) {
        console.log('AUTH_MIDDLEWARE: Auth not required and token missing. Skipping auth.'); // LOG 5
        return next();
      }
      ...
      console.log('AUTH_MIDDLEWARE: verifyAccessToken returned:', decoded ? 'Decoded' : 'Null'); // LOG 7

      if (!decoded) {
        console.log('AUTH_MIDDLEWARE: Invalid or expired access token. Returning 401.'); // LOG 8
        return res.status(401).json({
          success: false,
      ...
      console.log(`AUTH_MIDDLEWARE: Decoding successful, user ID: ${decoded.id}. Fetching user from DB...`); // LOG 9
      ...
      console.log('AUTH_MIDDLEWARE: Prisma findUnique returned user:', user ? user.id : 'None'); // LOG 10

      // Kiểm tra user tồn tại và active
      if (!user || user.status !== 'active') {
        console.log('AUTH_MIDDLEWARE: User not found or inactive. Returning 401.'); // LOG 11
        return res.status(401).json({
          success: false,
      ...
      if (options?.allowedUserTypes && !options.allowedUserTypes.includes(user.user_type)) {
        console.log(`AUTH_MIDDLEWARE: User type '${user.user_type}' not allowed. Returning 403.`); // LOG 12
      ...
      if (options?.requireMainConsultant && !user.is_main_consultant) {
        console.log('AUTH_MIDDLEWARE: Main consultant required but user is not. Returning 403.'); // LOG 13
        return res.status(403).json({
          success: false,
      ...
      const programTypeMap: Record<string, 'Aptech' | 'Arena' | 'Short-term + Steam'> = {
        Aptech: 'Aptech',
        Arena: 'Arena',
      ...
      if (
        ...
        console.log(`AUTH_MIDDLEWARE: Program type '${user.program_type}' not allowed. Returning 403.`); // LOG 14
        return res.status(403).json({
          success: false,
      ...
      req.user = {
        ...
        program_type: user.program_type ? programTypeMap[user.program_type] : null
      };

      console.log(`AUTH_MIDDLEWARE: User ${req.user.id} authenticated successfully. Calling next().`); // LOG 15
      next();
    } catch (error) {
      console.error('AUTH_MIDDLEWARE: CATCH BLOCK - Unexpected error during authentication:', error); // LOG 16
      res.status(500).json({
        success: false,
...
export const programCounselorOnly = (programType: 'Aptech' | 'Arena' | 'Short-term + Steam') => {
  return authenticateToken({
    allowedUserTypes: ['counselor'],
